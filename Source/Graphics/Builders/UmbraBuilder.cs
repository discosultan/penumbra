...
namespace Penumbra.Graphics.Builders
{
    internal class UmbraBuilder
    {
        ...
        private bool _startedMiddleOfFirstSegment = true;
        ...
        private bool _isSegmentActive;
        private bool _isFirstSegment;
        private Vector2 _prevRightToPointDir;
        public void ProcessHullPoint(Light light, Hull hull, ref HullPointContext context)
        {
            ProcessPoint(ref context);

            bool isLast = context.Index >= hull.TransformedPoints.Count - 1;
            if (isLast)
            {
                foreach (HullPointContext ctx in _firstSegmentBuffer)
                {
                    HullPointContext context2 = ctx;
                    ProcessPoint(ref context2);
                }
            }                            
            ...
            //            }
            //        }
            //        _startedMiddleOfFirstSegment = false;
            //        break;
            //    case Side.Backward:
            //        if (_startedMiddleOfFirstSegment)
        ...
        private void ProcessPoint(ref HullPointContext context)
        {
            if (context.Side == Side.Forward)
            {
                _isFirstSegment = false;
                _isSegmentActive = false;
            }
            else if (context.Side == Side.Backward)
            {
                if (_isFirstSegment)
                {
                    _firstSegmentBuffer.Add(context);
            ...
            else if (context.RightSide == Side.Right)
            {
                if (_isSegmentActive)
                {
                    if (VectorUtil.IsADirectingRightFromB(ref _prevRightToPointDir, ref context.LightRightToPointDir))
                    {
                        _activeSegment.Add(context);
                    }
                    else
                    {
                        StartNewSegment(ref context);
                    }
                }
                else
                {
                    ...
                    _isSegmentActive = true;
                }
            }
            else if (context.LeftSide == Side.Left)
            {
                if (_isFirstSegment)
                {
                    _firstSegmentBuffer.Add(context);
        ...
        private void StartNewSegment(ref HullPointContext context)
        {
            _activeSegment = new List<HullPointContext>();
            _segments.Add(_activeSegment);
            _activeSegment.Add(context);
            _prevRightToPointDir = context.LightRightToPointDir;
        }
        ...
        {                        
            ...
            _hullIndices.Clear();
            _isSegmentActive = false;
            _startedMiddleOfFirstSegment = true;
            _isFirstSegment = true;
            _segments.Clear();
        }
